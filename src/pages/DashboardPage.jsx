...
function DashboardPage({ user }) {
    ...
    const fetchHistoryRecommendations = async (token) => {
        try {
            ...
            if (response.ok) {
                ...
                console.error("Failed to fetch history recommendations.");
            }
        } catch (error) {
    ...
    useEffect(() => {
        const initializeDashboard = async () => {
            ...
            setCurrentPage(1);
            fetchBooks(token, searchTerm, genreFilter, 1, false); // Fetch page 1
    ...
    const handleUpdateStatus = async (bookId, status) => {
        ...
        if (response.ok) {
            fetchPendingBooks(token);
            fetchBooks(token, searchTerm, genreFilter, 1, false); // Refetch books from page 1
        }
    };
    ...
    const handleLoadMore = async () => {
        ...
        const token = (await supabase.auth.getSession())?.data?.session?.access_token;
        if (token) {
            fetchBooks(token, searchTerm, genreFilter, nextPage, true); // Fetch next page and append
